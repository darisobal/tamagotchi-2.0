#!/usr/bin/env node
/**
 * Creates a Supabase project, applies schema, disables email confirmation,
...
async function disableEmailConfirmation(ref) {
  console.log('Disabling email confirmation (for testing)...');
  try {
    ...
    console.warn('Disable manually: Authentication → Providers → Email → Confirm email OFF');
    console.warn(String(err));
  }
}
